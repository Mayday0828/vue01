  ...
  methods: {
    ...
    delBatch() {
      ...
      this.$confirm('您确认批量删除这些数据吗？', '确认删除', {type: "warning"}).then(response => {
        this.$request.delete('/index/notice/delete/batch', { data: this.ids }).then(res => {
    ...
    del(id) {
      this.$confirm('您确认删除吗？', '确认删除', {type: "warning"}).then(response => {
        this.$request.delete('/index/notice/delete/' + id).then(res => {
          if (res.code === '200') {   // 表示操作成功
            this.$message.success('操作成功')
            this.load(1)
          } else {
    ...
    load(pageNum) {  // 分页查询
      ...
      this.$request.get('/index/notice/selectByPage', {
        params: {
